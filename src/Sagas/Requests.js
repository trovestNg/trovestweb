...
export const getSuperAdminDashboard = (payload) => {
  ...
  const requestOptions = {
    method: "get",
    url: `${baseUrl}/super/dashboard?agent_page=${page}&agent_limit=${limit}`,
    headers: headers,
  };
  return axios(requestOptions);
};

export const superCreateAdmin = (data, token) => {
  headers.authorization = `Bearer ${token}`;
  headers["Content-Type"] = "multipart/form-data";
  const requestOptions = {
    method: "post",
...
export const adminGetAgents = (token, agentId) => {
  headers.authorization = `Bearer ${token}`;
  const requestOptions = {
    method: "get",
    url: `${baseUrl}/admin/agent/${agentId}`,
    headers: headers,
  };
  return axios(requestOptions);
};

export const adminGetAgent = (token, agentId) => {
  headers.authorization = `Bearer ${token}`;
  const requestOptions = {
    method: "get",
    url: `${baseUrl}/admin/agent/${agentId}`,
...
export const adminGetAgentArtisans = (token, agentId, page, limit) => {
  headers.authorization = `Bearer ${token}`;
  const requestOptions = {
    method: "get",
    url: `${baseUrl}/admin/agent-artisans/${agentId}?page=${page}&limit=${limit}`,
    headers: headers,
  };
  return axios(requestOptions);
};

export const adminGetArtisan = (token, agentId) => {
  headers.authorization = `Bearer ${token}`;
  const requestOptions = {
    method: "get",
    url: `${baseUrl}/admin/artisan/${agentId}?limit=20`,
    headers: headers,
  };
  return axios(requestOptions);
};

export const adminGetArtisans = (token, agentId) => {
  headers.authorization = `Bearer ${token}`;
  const requestOptions = {
    method: "get",
    url: `${baseUrl}/admin/artisan/${agentId}?limit=20`,
...
export const resetAdminPassword = (payload) => {
  ...
  const requestOptions = {
    method: "patch",
    url: `${baseUrl}/admin/forgot-password/${token}`,
    data: { "password": pass },
    headers: headers,
  };
  return axios(requestOptions);
...
export const adminUpdateBio = (payload) => {
  console.log("updated to :", payload);
  const { artisanId, body, token } = payload;
  headers.authorization = `Bearer ${token}`;
  headers["Content-Type"] = "application/json";
