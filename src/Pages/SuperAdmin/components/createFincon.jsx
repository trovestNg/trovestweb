...
import { Button, FormControl, InputGroup, Modal,Spinner, ModalBody, Col, Row, Form } from "react-bootstrap";
import { Formik } from "formik";
import * as yup from 'yup';
import Compress from "react-image-file-resizer";
import { user_storage_token } from "../../../config";
import { createFinconAccount } from "../../../Sagas/Requests";
import DisplayMessage from "../../../Components/Message";


const CreateFincon = ({ on, off, check, success, offSuccess, onSuccess, fetchService }) => {
    ...
    async function createAdminAccount(value) {
        ...
        try {
            ...
            const response = await createFinconAccount(formdata, superAdminToken)
        ...
        {({ values, handleChange, handleSubmit, errors }) => (
            <>
                <Modal show={on} centered size='lg'>
                    ...
                    <ModalBody className="d-flex justify-content-center">
                        <Form className="w-75 gap-3 d-flex justify-content-center">
                            <Row className="d-flex flex-row w-100 m-0 w-100">
                                <Col>
                                    <InputGroup className='bg-transparent border  px-2 gap-2 border-secondary rounded d-flex justify-content-start align-items-center'>
                                        <i className="bi text-secondary bi-person-fill"></i>
                                ...
                                <InputGroup className='bg-transparent border  px-2 gap-2 border-secondary rounded d-flex justify-content-start align-items-center'>
                                <i className="bi bi bi-people-fill text-secondary"></i>
                            ...
                            <Row className="d-flex gap-2 m-0 flex-row w-100 mt-3  w-100">
                                <Col>
                                    ...
                                    <i className="bi bi-lock-fill text-secondary"></i>
                                        ...
                                        <i className={secureText ? "bi bi-eye-fill text-secondary" : "bi bi-eye-slash-fill text-secondary"} onClick={() => setSecureText(!secureText)}></i>
                                    </InputGroup>
                                </Col>
                                <Col>
                                    <InputGroup className='bg-transparent border  px-2 gap-2 border-secondary rounded d-flex justify-content-start align-items-center'>
                                    <i className="bi bi-lock-fill text-secondary"></i>
                                        ...
                                        <i className={secureCText ? "bi bi-eye-fill text-secondary" : "bi bi-eye-slash-fill text-secondary"} onClick={() => setSecureCText(!secureCText)} ></i>
                                    </InputGroup>
                                </Col>
                            </Row>
                            ...
                            <Row className="d-flex m-0 flex-row w-100 mt-3  w-100">
                                <Col>
                                    <InputGroup className='bg-transparent border  gap-2 border-secondary rounded d-flex justify-content-start align-items-center'>
                            ...
                            <Row className="d-flex m-0 flex-row mt-3 w-100">
                                ...
                                <Col>
                                    <InputGroup>
                                        <Button className="bg-transparent border border-secondary text-dark" type="rest" onClick={off}>Cancel</Button>
                                    </InputGroup>
                                </Col>
                            </Row>
                        </Form>
                ...
                <Modal show={success} centered>
                    <Modal.Header className='bg-secondary text-light py-0' style={{ fontFamily: 'Montserrat-Bold' }}>
