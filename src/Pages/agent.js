...
export default function Dashboard() {
    ...
    const fetch = async () => {
        { dashBoard && toast.success(operations[0].name); setDashBoard(false) }
        { addAdmin && toast.success(operations[1].name); setAdmin(false) }
        { addAgent && toast.success(operations[2].name); setAddAgent(false) }
        { addFinCon && toast.success(operations[3].name); setAddFinCon(false) }
        { paymentReq && toast.success(operations[4].name); setPaymentReq(false) }
        { profile && toast.success(operations[5].name); setProfile(false); }
        { checkToken() }
