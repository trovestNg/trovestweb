...
const SureToRejectSingleBmoModal: React.FC<any> = ({ show, Id, off, clickedOwner }) => {
    ...
    const deleteBo = async (body: any) => {
       try {
        ...
        const res = await api.post(`reject/single`, delBody, `${userInfo?.access_token}`)
        if (res?.status==200) {
            setLoading(false);
            toast.success('Single BO succesfully Rejected');
            off();
        }
            ...
            <Modal show={show} centered>
                ...
                <Modal.Body>
                    <div className="py-2 d-flex justify-content-center align-items-center flex-column">
                        ...
                        <p className="text-center px-3">You are about to reject creation of this single BO request.</p>
...
export default SureToRejectSingleBmoModal;