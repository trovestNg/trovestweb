...
const EditAgentInfoModal: React.FC<any> = ({ show, off, agentInfo, agentId}) => {
    ...
    const initialVal = {
        ...
        first_name: agentInfo?.first_name || 'ade',
    ...
    const createNewAgent = async (body: any) => {
        console.log(body)
         ...
         try {
            ...
            setLoading(true)
            const res = await api.patch(`admin/update-agent/${agentId}`,body,token);
            if(res?.data?.success){
                toast.success('Agent info updated succesfully');
                setLoading(false);
                setFileName('');
                setFileUrl('')
                off();
        ...
        <div>
            <Modal size="lg" show={show} centered>
                <Modal.Header className="d-flex justify-content-between"
                    ...
                    <p className="p-0 m-0 text-secondary">Update Agent Info</p>
