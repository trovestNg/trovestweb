...
import { IAdmin } from "../../pages/super-admin/superadmin-dashboardpage";

const AdminPagination: React.FC<any> = ({ data }) => {
    console.log({seemehere:data})
            ...
            <table className="table table-striped border border-1 w-100">
                <thead className="thead-dark">
                    <tr >
                        ...
                        <th scope="col" className="bg-primary text-light">No of agents</th>
                    </tr>
                </thead>
                <tbody>
                    {currentItems.length <= 0 ? <tr><td className="text-center" colSpan={5}>No Data Available</td></tr> :
                        currentItems.map((agent: IAdmin, index: number) => (
