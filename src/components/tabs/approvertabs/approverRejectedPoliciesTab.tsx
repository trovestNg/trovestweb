...
import AuthorizerRejectedPolicyPagination from "../../paginations/authorizer/authorizer-rejected-policy-pagiantion";
    ...
    const getUploadedPolicies = async () => {
        ...
        try {
            ...
            if (userInfo) {
                ...
                if (res?.data) {
                    let rejectedPolicies = res?.data.filter((policy: IPolicy) => policy.isRejected && !policy.isDeleted && !policy.markedForDeletion)
            ...
            <div className="mt-4" >
                {
                    loading ? <table className="table table-stripped w-100">
                        ...
                        <AuthorizerRejectedPolicyPagination data={policies} refData={()=>setRefreshData(!refreshData)}/>
                }
            </div>
            {/* {
                policies.length <= 0 ? '' :
