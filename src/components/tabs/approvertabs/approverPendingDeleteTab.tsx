...
import DeletePolicyConfModal from "../../modals/deletePolicyConfModal";
import AuthPendingDelPolicyPagination from "../../paginations/authorizer/auth-pending-del-policy-pagiantion";
...
const ApproverPendingDeleteTab: React.FC<any> = ({ handleCreatePolicy }) => {
    ...
    const handlePolicyDelete = async (e: any) => {
        ...
        if (userInfo) {
            let userName = userInfo?.profile?.sub.split('\\')[1]
            const res = await api.post(`Policy/delete/approve`, { "ids": [policyId], "username": userName }, userInfo?.access_token);
            if (res?.status == 200) {
            ...
            <div className="mt-4" >
                {
                    loading ? <table className="table table-stripped w-100">
                        ...
                        <AuthPendingDelPolicyPagination data={policies} refData={()=>setRefreshData(!refreshData)}/>
                }
            </div>
            {/* {
                policies.length <= 0 ? '' :
